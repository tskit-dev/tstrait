...
class Test_sim_phenotype_output_dim:
    ...
    def test_binary_mutation_model(self):
        ...
        model = trait_model.TraitModelAdditive(0,1)
        ts = msprime.sim_ancestry(num_ind, sequence_length=100_000, random_seed=random_seed)
        ts = msprime.sim_mutations(ts, rate=0.01, random_seed=random_seed, model="binary")
        phenotype_result, genetic_result = simulate_phenotype.sim_phenotype(ts, num_causal, model, h2, random_seed)
    ...
    def test_output_dim_Allele(self, num_ind, num_causal, h2, random_seed):
        ...
        assert max(genetic_result.allele_frequency < 1) and min(genetic_result.allele_frequency > 0)
...
class Test_sim_genetic_value:
    def tree_sequence(self):
        ...
        rng = np.random.default_rng(1)
        simulator = simulate_phenotype.PhenotypeSimulator(ts, num_causal=6, h2= 0,
                                                              model = model, rng = rng)
        ...
        assert np.array_equal(genotypic_effect_data.effect_size, np.array([2,2,2,2,2,2]))
        assert np.array_equal(genotypic_effect_data.causal_state, np.array(["T","T","C","T","C","C"]))
        assert np.allclose(genotypic_effect_data.allele_frequency, np.array([0.5,0.75,0.25,0.25,0.25,0.5]))