...
class Test_sim_phenotype_input:
    ...
    def test_ts(self, ts):
        ...
        with pytest.raises(TypeError, match="Input must be a tree sequence data"):
            tstrait.sim_traits(ts=ts, num_causal=3, model=model, alpha=1, random_seed=1)
...
class Test_tree_sequence:
    ...
    def test_tree_sequence(self):
        ...
        assert np.array_equal(
            sim_result["CausalState"], np.array(["T", "T", "C", "T", "C", "C"])
        )
        assert np.isclose(
            sim_result["EffectSize"][0], 2 * self.const(freq=0.5, alpha=-1)
        )
        assert np.isclose(
            sim_result["EffectSize"][1], 2 * self.const(freq=0.75, alpha=-1)
        )
        assert np.isclose(
            sim_result["EffectSize"][2], 2 * self.const(freq=0.25, alpha=-1)
        )
        assert np.isclose(
            sim_result["EffectSize"][3], 2 * self.const(freq=0.25, alpha=-1)
        )
        assert np.isclose(
            sim_result["EffectSize"][4], 2 * self.const(freq=0.25, alpha=-1)
        )
        assert np.isclose(
            sim_result["EffectSize"][5], 2 * self.const(freq=0.5, alpha=-1)
        )
